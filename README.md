...
- **CI / deploy.** `.github/workflows/rbmc.yml` runs on every push to the `rbmc` branch: typecheck + the full test suite (integration tests included, against a Postgres service), then builds and pushes `paschendale/sentinel-rbmc:latest` (and a `sha-…` tag). The branch never merges into `main` and never goes through semantic-release. The `sentinel-rbmc` deployment (`disbelief/sentinel-rbmc/docker-compose.yml` in the paschendale.net repo) tracks that image and What's Up Docker rolls it out.
