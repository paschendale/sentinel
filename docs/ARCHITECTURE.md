...
- Per-request limits: `ctx.http`/`ctx.s3` combine the run signal with `AbortSignal.timeout(options.timeout ?? remaining budget)` via `AbortSignal.any`, and read bodies as streams so a timeout reports its progress
