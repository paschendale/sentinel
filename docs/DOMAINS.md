...
interface RequestOptions {
  headers?: Record<string, string>
  timeout?: number   // per-request limit, ms, covering headers and full body; defaults to what is left of timeout_ms
...
- `ctx.http` routes through undici. Each request is limited by `options.timeout`, or by what is left of the test's `timeout_ms` when omitted; the limit covers the response headers and the full body. A request over its limit throws `HttpRequestError` with `code: 'HTTP_TIMEOUT_ERROR'`, and the message says whether headers had arrived and how much of the body was read. Other failures throw `code: 'HTTP_FETCH_ERROR'` or `'HTTP_REDIRECT_ERROR'`. A non-positive `timeout` throws `TypeError`
